...
class BlumTod:
    ...
    def get_location(self):
        ...
        try:
            response = requests.get(url='https://2ip.ru', headers=self.base_headers, proxies=proxy_headers)
    ...
    def http(self, url, headers, data=None, proxy=None):
        while True:
            proxy_headers = {
                'https': self.current_account_proxy
            }
            try:
                if data is None:
                    ...
                    res = requests.get(url, headers=headers, proxies=proxy_headers, timeout=30)
                ...
                if data == "":
                    res = requests.post(url, headers=headers, proxies=proxy_headers, timeout=30)
                    open(".http_request.log", "a", encoding="utf-8").write(
                        res.text + "\n"
                    )
                    if "<html>" in res.text:
                ...
                res = requests.post(url, headers=headers, data=data, proxies=proxy_headers, timeout=30)
